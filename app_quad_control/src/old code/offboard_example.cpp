...
//
// Does Offboard control using Global (Latitude, Longitude, relative altitude)
// co-ordinates.
//
// returns true if everything went well in Offboard control
//
bool offb_ctrl_pos_global(mavsdk::Offboard &offboard,
  ...
  // Send it once before starting offboard, otherwise it will be rejected.
  // this is a step north about 10m, using the default altitude type (altitude
  // relative to home)
  const Offboard::PositionGlobalYaw north{origin.latitude_deg + 0.0001,
                                          origin.longitude_deg, 20.0f, 0.0f};
  ...
  const Offboard::PositionGlobalYaw home{
      origin.latitude_deg, origin.longitude_deg, origin.altitude_m + 10.0f,
      180.0f, Offboard::PositionGlobalYaw::AltitudeType::Amsl};
